...
case $1 in
  ...
  ;;
  spacetotal)
    echo $($ceph_bin df |awk '{print $1}'|sed '3!d')
  ;;
  spaceavail)
    #echo $($ceph_bin df |awk '{print $2}'|grep G|cut -d"G" -f1) not working
    echo $($ceph_bin df |awk '{print $3}'|sed '3!d')
  ;;
  poolspaceavail)
  ...
  osdspaceavail)
    ...
    echo $(sudo df -h |grep ceph|awk '{print $4}'|cut -d'G' -f1)
  ;;
  osdcount)
